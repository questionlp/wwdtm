...
* Added ``mode`` and ``mode_multiple`` to the ``scoring`` and ``scoring_decimal`` dictionary returned in :py:meth:`wwdtm.panelist.PanelistStatistics.retrieve_statistics_by_id` and :py:meth:`wwdtm.panelist.PanelistStatistics.retrieve_statistics_by_slug`
