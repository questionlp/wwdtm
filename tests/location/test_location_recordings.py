...
@pytest.mark.parametrize("location_id, exclude_null_dates", [(95, True), (95, False)])
def test_location_recordings_retrieve_recordings_by_id(
    location_id: int, exclude_null_dates: bool
):
    ...
    recording = recordings.retrieve_recordings_by_id(location_id, exclude_null_dates)
...
@pytest.mark.parametrize(
    "location_slug, exclude_null_dates",
    [
        ("the-chicago-theatre-chicago-il", True),
        ("the-chicago-theatre-chicago-il", False),
    ],
)
def test_location_recordings_retrieve_recordings_by_slug(
    location_slug: str, exclude_null_dates: bool
):
    ...
    """
    recordings = LocationRecordings(connect_dict=get_connect_dict())
    recording = recordings.retrieve_recordings_by_slug(
        location_slug, exclude_null_dates
    )
